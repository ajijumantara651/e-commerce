                    ...
                    <table class="table table-striped table-bordered table-hover">
                      <thead>
                        <tr>
                          ...
                          <th width="178">
                            <div align="center">Tanggal Pesan</div>
                          </th>
                          <th width="178">
                            <div align="center">Harga</div>
                          </th>
                          <th width="178">
                            <div align="center">Foto Produk</div>
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php 
                          ...
                          $total_data = "SELECT COUNT(*) AS jumData FROM `tbl_produk` INNER JOIN `tbl_kategori` ON `tbl_produk`.`Kd_Kategori` = `tbl_kategori`.`Kd_Kategori` INNER JOIN `tbl_supplier` ON `tbl_produk`.`Kd_Supplier` = `tbl_supplier`.`Kd_Supplier` INNER JOIN `tbl_pesanan` ON `tbl_produk`.`Kd_Barang` = `tbl_pesanan`.`Kd_Barang` INNER JOIN `tbl_customer` ON `tbl_pesanan`.`Kd_Customer` = `tbl_customer`.`Kd_Customer` ORDER BY `id_barang`";

                          $query = "SELECT * FROM `tbl_produk` INNER JOIN `tbl_kategori` ON `tbl_produk`.`Kd_Kategori` = `tbl_kategori`.`Kd_Kategori` INNER JOIN `tbl_supplier` ON `tbl_produk`.`Kd_Supplier` = `tbl_supplier`.`Kd_Supplier` INNER JOIN `tbl_pesanan` ON `tbl_produk`.`Kd_Barang` = `tbl_pesanan`.`Kd_Barang` INNER JOIN `tbl_customer` ON `tbl_pesanan`.`Kd_Customer` = `tbl_customer`.`Kd_Customer` ORDER BY `id_barang` LIMIT $offset, $dataPerPage";
                        ...
                        <tr>
                          ...
                          <td><div align="center"><?php echo $data['Nama_Supplier'] ?></div></td>
                          <td><div align="center"><?php echo $data['Customer'] ?></div></td>
                          <td><div align="center"><?php echo $data['Tanggal_Pesan'] ?></div></td>
                          <td><div align="center"><?php echo number_format($data['Harga_Jual'],0,',','.') ?></div></td>
                          <td><div align="center"><img src="produk/<?php echo $data['Foto'] ?>" width="100"></div></td>
                          <td align="center">
                          </td>
                        </tr>
